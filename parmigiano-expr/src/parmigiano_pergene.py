...
from models import parmigiano_expression, simulate_expression
...
def _extract_tau_history_step(epoch):
...
def main():
    ...
    for run in range(config['refits']):
        print("\nREFIT #",run+1)
        # Fit
        losses, times, posterior_stats, beta_samples, mu_samples, tau_history, simulations = \
            fit_parmigiano(data_train, config)
